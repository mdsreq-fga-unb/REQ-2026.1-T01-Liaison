...
from .models import StudentProfile, OrganizationProfile, StudentGalleryPhoto, OrgGalleryPhoto
...
class CustomTokenObtainPairSerializer(TokenObtainPairSerializer):
    def __init__(self, *args, **kwargs):
        ...
        # Add cnpj field for organization login
        self.fields["cnpj"] = serializers.CharField(required=False)
        # Make email optional (orgs login by CNPJ instead)
    ...
    def validate(self, attrs):
        ...
        if cnpj:
            ...
            try:
                org = OrganizationProfile.objects.get(cnpj=cnpj_digits)
            ...
            # Replace cnpj with the org's email for parent authentication
            attrs = {k: v for k, v in attrs.items() if k != "cnpj"}
            attrs["email"] = org.user.email

        elif not email:
            raise serializers.ValidationError("Informe o e-mail ou CNPJ para login.")
